...
/** Compact Wear-tegel: toont pin-set (Werk/Thuis/Onderweg), tik wisselt. */
class SounmaxPinSetTileService : TileService() {
    ...
    override fun onTileRequest(requestParams: RequestBuilders.TileRequest): ListenableFuture<TileBuilders.Tile> {
        return scope.future {
            ...
            if (!cmd.isNullOrBlank()) {
                runCatching { WearClient.send(this@SounmaxPinSetTileService, cmd) }
            }
            val pin = readPinSet()
                    ...
                    TimelineBuilders.Timeline.Builder()
                        .addTimelineEntry(
                            TimelineBuilders.TimelineEntry.Builder()
                                .setLayout(Layout.Builder().setRoot(buildLayout(pin, params)).build())
                                .build()
                        )
                        .build()
                )
    ...
    private fun buildLayout(
        ...
        val label = pin.ifBlank { "Set" }
        val click = Clickable.Builder()
            .setOnClick(
                ActionBuilders.LoadAction.Builder()
                    .setRequestState(
                        StateBuilders.State.Builder()
                            .addKeyValuePair(STATE_CMD, ActionBuilders.stringVal(WearPaths.CMD_CYCLE_PIN_SET))
                            .build()
                    )
                    .build()
            )
            .setId(WearPaths.CMD_CYCLE_PIN_SET)
            .build()
    ...
    private suspend fun readPinSet(): String {
        return runCatching {
            val items = Wearable.getDataClient(this).dataItems.await()
            val match = items.find { it.uri.path == WearPaths.STATUS }
            items.release()
            if (match == null) "" else {
                DataMapItem.fromDataItem(match).dataMap.getString(WearPaths.KEY_PIN_SET) ?: ""
            }
        }.getOrElse { "" }
    }
