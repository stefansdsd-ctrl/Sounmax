...
object SceneAutomation {
    ...
    fun writeSuggested(context: Context) {
        ...
        var scene = ListeningScenes.suggestedNow(weekDose)
        var next = AutoModeFusion.adjust(context, scene)
        scene = SceneReason.step(scene, next, "auto-fusie", reasons)
        next = SceneScheduleAdvisor.adjust(context, scene)
        scene = SceneReason.step(scene, next, "tijdschema", reasons)
        next = WeatherAdvisor.suggest(context, scene)
        ...
        next = OutdoorSafetyAdvisor.adjust(context, scene)
        scene = SceneReason.step(scene, next, "buiten-veilig", reasons)
        ...
        scene = SceneReason.step(scene, next, "schoolochtend", reasons)
        next = EveningWindDownAdvisor.adjust(context, scene)
        scene = SceneReason.step(scene, next, "avondrust", reasons)
        next = RssiCodecAdvisor.adjust(context, scene)
        scene = SceneReason.step(scene, next, "zwak BT-signaal", reasons)
        next = ContentSceneAdvisor.adjust(scene)
        scene = SceneReason.step(scene, next, "nu-spelend", reasons)
        next = PhoneBatteryAdvisor.adjust(context, scene)
        scene = SceneReason.step(scene, next, "telefoonaccu", reasons)
        val battery = prefs.getInt("last_battery", -1).takeIf { it in 0..100 }
        next = BatteryPowerAdvisor.adjust(context, scene, battery)
        scene = SceneReason.step(scene, next, "headset-accu", reasons)
        SceneReason.save(prefs, reasons)
